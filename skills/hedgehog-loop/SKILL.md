...
Every layer scaffolds from its own generator in `tools/generators/` —
schema file, table registration, barrel wiring, and the layer's
conventional shape all land in one deterministic step. What's authored on
top is the entity-specific delta: the field list and its types, the
module's validation and mutation rules, and the screen's layout and
interaction pattern. See [scaffolding a layer](references/scaffolding.md)
for the generator commands and flag contract.
...
See [scaffolding a layer](references/scaffolding.md) for the generator
commands, the full flag contract (including `entity --layer` matched
against a claimed task), and why this core needs no workspace-wiring step
the way a multi-package Nx workspace does.
